...
Data for exercises comes as part of the `faraway` package. 
...
The `coagulation` dataset comes from a study of blood coagulation times. Twenty-four animals were randomly assigned to one of four diets, and the samples were taken in a random order. 

a) Plot the data and comment
b) Fit a fixed effect model and comment
c) Construct a prediction for the response of a new animal assigned to diet D
d) Fit a random effect model using REML. A new animal is assigned to diet D. Predict the blood coagulation time for this animal. 
e) Compare the two models using LRT.
...
A number of growers supply broccoli to a food processing plant. The plant instructs the growers to pack the broccoli into standard-size boxes. There should be 18 clusters of broccoli per box. Because the growers use different varieties and methods of cultivation, there is some variation in the cluster weights. The plant managers selected three growers at random then four boxes at random supplied by these growers. Three clusters were selected from each box. The data may be found in the `broccoli` dataset. The weight in grams of the cluster is given. 

a) Plot the data and comment on the nature of variation seen. 
b) Compute the mean weights within growers. Compute the mean weights withing boxes. 
...
An experiment was conducted to determine the effects of recipe and baking temperature on chocolate cake quality. Fifteen batches of cake mix for each recipe were prepared. Each batch was sufficient for six cakes. Each of the six cakes was baked at different temperature which was randomly assigned. Several measures of cake quality were recorded of which breaking angle was just one. The dataset is called `choccake`.
...
d) Test for recipe effect. 
e) Check the diagnostic plots and comment: i) the residuals vs. fitted values, ii) QQ plot of the residuals, and iii) QQ plot of the batch random effects. 
